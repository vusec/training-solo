...
python3 /scanner/run-parallel.py /inspectre-gadget/inspectre $DIR_WORK/images/dump_vmlinux $DIR_WORK/entry-points/collision_targets.csv -c /scanner/config_all.yaml -o out -t360 -j32 -s $DIR_WORK/images/vmlinux-dbg
# Move all to output folder
mv fail.txt out
mv unsupported.txt out || true
# Merge all results.
cd out
...
mkdir -p lists
cp /work-dir/entry-points/branches_kernel_text.csv lists/
cp /work-dir/entry-points/collisions.csv lists/
