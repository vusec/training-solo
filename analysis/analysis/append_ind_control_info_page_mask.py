...
def main(victims_csv, in_csv, out_csv):
    ...
    victims_df['page_mask'] = 0xdead
    gadgets_df['page_mask'] = 0xdead

    # exploitable_gadgets = victims_df[ (victims_df['exploitable'] == 'True') & (victims_df['base_has_indirect_secret_dependency'] == 'False')]

    if IS_TFP:
        exploitable_gadgets = gadgets_df[ (gadgets_df['exploitable'] == True)]
    else:
        exploitable_gadgets = gadgets_df[ (gadgets_df['exploitable'] == 'True') & (gadgets_df['base_has_indirect_secret_dependency'] == 'False')]

    victims_page_mask = victims_df['page_mask']

    print("All gadgets in page_mask", gadgets_df[ gadgets_df['page_mask'].isin(victims_page_mask)]['pc'].nunique())
    print("Expl gadgets in page_mask", exploitable_gadgets[ exploitable_gadgets['page_mask'].isin(victims_page_mask)]['pc'].nunique())
    ...
    print(f"   [+] {'Number of unique victim PAGE MASK:':<55} {len(victims_df['page_mask'].unique())}")
    print("")
    print(f"   [+] {'Number of unique victim PCs with direct control:':<55} {len(controlled_direct_victims_df['pc'].unique())}")
    print(f"   [+] {'Number of unique victim PAGE MASK with direct control:':<55} {len(controlled_direct_victims_df['page_mask'].unique())}")
    print("")
    print(f"   [+] {'Number of unique victim PCs with indirect control:':<55} {len(controlled_indirect_victims_df['pc'].unique())}")
    print(f"   [+] {'Number of unique victim PAGE with indirect control:':<55} {len(controlled_indirect_victims_df['page_mask'].unique())}")

    print("")
    print(f"   [+] {'Number of unique victim PCs with any control:':<55} {len(controlled_all_victims_df['pc'].unique())}")
    print(f"   [+] {'Number of unique victim PAGE MASK with any control:':<55} {len(controlled_all_victims_df['page_mask'].unique())}")
    print(f"   [+] {'Number of unique page_masks gadgets:':<55} {len(gadgets_df['page_mask'].unique())}")

    gadgets_wo_controlled_victim =  gadgets_df.loc[ ~gadgets_df['page_mask'].isin(controlled_all_victims_df['page_mask'])]
    gadgets_w_controlled_victim =  gadgets_df.loc[ gadgets_df['page_mask'].isin(controlled_all_victims_df['page_mask'])]
    gadgets_w_victim =  gadgets_df.loc[ gadgets_df['page_mask'].isin(victims_df['page_mask'])]
    print("")
    print(f"   [+] {'Number of unique gadgets (PC):':<55} {  len(gadgets_df['pc'].unique()) }")
    print(f"   [+] {'Number of unique pc WITHOUT a controlled victims:':<55} {  len(gadgets_wo_controlled_victim['pc'].unique()) }")
    print(f"   [+] {'Number of gadgets WITH controlled victims:':<55} {  len(gadgets_w_controlled_victim) }")
    print(f"   [+] {'Number of unique PC WITH controlled victims:':<55} {  len(gadgets_w_controlled_victim['pc'].unique()) }")
    # print(f"   [+] {'Number of unique dir_address WITH controlled victims:':<55} {  len(gadgets_w_controlled_victim['dir_address'].unique()) }")

    print(f"[-] Grouping...")

    victims_by_pc = victims_df[['page_mask', 'pc', 'name', 'uuid', 'controlled_sufficiently', 'controlled_sufficiently_indirect', 'controlled_sufficiently_all', 'controlled']].groupby('page_mask').agg(lambda x: set(y for y in x if y))
    victims_by_pc['n_name'] = victims_df[['pc', 'page_mask', 'name', 'uuid', 'controlled_sufficiently', 'controlled']].groupby('page_mask').name.nunique()


    # gadgets_w_victim = pd.merge(gadgets_w_victim, victims_by_pc, left_on='page_mask', right_on='page_mask', suffixes=('', '_victim'))
    ...
    gadgets_grouped_df = gadgets_w_victim.groupby(['page_mask', 'controlled_requirements_regs_groups'])
    ...
    victims_grouped_df = controlled_all_victims_df[['pc', 'page_mask', 'name', 'uuid', 'controlled_sufficiently', 'controlled_sufficiently_indirect', 'controlled_sufficiently_all', 'controlled']].groupby(['page_mask', 'controlled_sufficiently', 'controlled_sufficiently_indirect', 'controlled_sufficiently_all'])



    required_regs_non_matching = set()

    for (ind_page_mask, required_regs_groups,), gadgets in gadgets_grouped_df:

        victims_matching = {'uuid' : set(), 'pc' : set(), 'name' : set(), 'regs' : set(), 'regs_suf' : set(), 'regs_matching' : set()}

        for (victim_page_mask, controlled_sufficiently, controlled_sufficiently_indirect, controlled_sufficiently_all), victims in victims_grouped_df:
            if victim_page_mask != ind_page_mask:
                continue
            ...
            victims_matching['name'].update(set(victims['name']))
            victims_matching['pc'].update(set(victims['pc']))
            # victims_matching['page_mask'] = victims['page_mask']
        ...
        # gadgets['victims_pc'] = str(victims_matching['pc'])
        ...
        gadgets['victims_uuid'] = ''
        gadgets['victims_name'] = ''
        gadgets['victims_pc'] = ''
        gadgets['victims_regs'] = ''
        gadgets['victims_regs_suf'] = ''
        gadgets['victims_regs_matching'] = str(victims_matching['regs_matching'])
        gadgets['n_victims_pc'] = len(victims_matching['pc'])
        gadgets['n_victims_name'] = len(victims_matching['name'])
        gadgets['n_victims_uuid'] = len(victims_matching['uuid'])
    ...
    print(f"   [+] {'Number of unique gadgets (PC):':<55} {  len(all_gadgets['pc'].unique()) }")
    ...
    print(f"   [+] {'Number of (rows):':<55} {  len(all_gadgets_w_controlled_victim) }")


    all_gadgets_w_controlled_victim.to_csv(out_csv, sep=';', index=False)
