...
def main(input, output, text_base):
    ...
    duplicated_addresses = df[df.duplicated(subset=['address'], keep=False)]
    ...
    for set_id in BTB_SET:
        ...
        df[f'set_{set_id}'] = df.apply(get_mask_of_pc, args=(set_mask, BTB_SET[set_id]['BTB_SET_START']), axis=1)
...
# -----------------------------------------------------------------------------
    ...
    columns_to_hex = ['a_address', 'b_address', 'a_pc', 'b_pc', 'a_target', 'b_target', 'target']
    if text_base:
