...
def main(input, branch_entry_points, output, text_base):
    ...
    print(f"{'# Kernel IND branches:':30} {len(df[(df['is_module'] == 0) & (df['is_module'] == 0)])}")
...
# -----------------------------------------------------------------------------
    ...
    for tag_id in BTB_TAG:
        ...
        for id, group in grouped:
            ...
            for collision_combo in combinations(group.iterrows(), 2):
                ...
                if ind_a['target_type'] == ind_b['target_type']:
                    continue
    ...
    ind_addresses = pd.concat([final[final['b_target_type'] == BRANCH_TYPE_INDIRECT]['b_address'],
                               final[final['a_target_type'] == BRANCH_TYPE_INDIRECT]['a_address']])


    dir_addresses = pd.concat([final[final['b_target_type'] == BRANCH_TYPE_DIRECT]['b_address'],
                               final[final['a_target_type'] == BRANCH_TYPE_DIRECT]['a_address']])
    ...
    print(f"{'Unique DIR branches:':30} {dir_addresses.nunique()}")
    ...
    for tag_id in BTB_TAG:
        ...
        print(f"{'  - Kernel branches:':30} {ind_addresses_tag_kernel.nunique()}")
        print(f"{'  - Module branches:':30} {ind_addresses_tag_module.nunique()}")
...
# -----------------------------------------------------------------------------
    ...
    if text_base:
        columns_to_hex += ['a_orig_address', 'b_orig_address', 'a_orig_pc', 'b_orig_pc']
