...
def main(victims_csv, in_csv, out_csv):
    ...
    gadgets_wo_controlled_victim =  gadgets_df.loc[ ~gadgets_df['ind_address'].isin(controlled_all_victims_df['pc'])]
    gadgets_w_controlled_victim =  gadgets_df.loc[ gadgets_df['ind_address'].isin(controlled_all_victims_df['pc'])]
    gadgets_w_victim =  gadgets_df.loc[ gadgets_df['ind_address'].isin(victims_df['pc'])]
    print("")
    ...
    print("")
    print(f"   [+] {'Number of gadgets (rows):':<55} {  len(gadgets_df) }")
    print(f"      [+] {'Number of gadgets (rows) exploitable != false:':<52} {  len(gadgets_df[ gadgets_df['exploitable'] != 'True']) }")
    if not IS_TFP:
        print(gadgets_df['transmitter'].value_counts())
    ...
    gadgets_w_victim = pd.merge(gadgets_w_victim, victims_by_pc, left_on='ind_address', right_on='pc', suffixes=('', '_victim'))
    ...
    gadgets_grouped_df = gadgets_w_victim.groupby(['ind_address', 'controlled_requirements_regs_groups'])
    ...
    print(f"   [+] {'Number of unique victims (ind_address) with matching gadget:':<55} {  len(all_gadgets_w_controlled_victim['ind_address'].unique()) }")
