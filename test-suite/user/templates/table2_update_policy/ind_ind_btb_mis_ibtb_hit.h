...
/**
 * This template tests if the BTB is updated if both the BTB and iBTB
 * contain an incorrect entry.
 *
 * For reference, single run hit results:
 * | CPU                    |   SHORT            NEAR             FAR
 * |------------------------|--------------------------------------------------|
 * | Intel Core i7-10700K   |      0 (  0.0%)  10000 (100.0%)      0 (  0.0%)
 * | Intel Core i7-11700    |      0 (  0.0%)  10000 (100.0%)      0 (  0.0%)
 * | Intel Core i9-14900K P |      0 (  0.0%)   9996 (100.0%)      4 (  0.0%)
 * | Intel Core 7 258V P    |      0 (  0.0%)   9992 ( 99.9%)      8 (  0.1%)
 */
...
void do_train_function(config * cfg)
{
    ...
    // Evict BTB entry (BTB=miss)
    evict_btb_entry_caller1(cfg);

    // Status: BTB = miss, iBTB = hit
    execute_path(cfg, &cfg->histories[P_VICTIM], &cfg->paths[P_TRAINING], &cfg->target0);
    execute_path(cfg, &cfg->histories[P_VICTIM], &cfg->paths[P_TRAINING], &cfg->target0);
