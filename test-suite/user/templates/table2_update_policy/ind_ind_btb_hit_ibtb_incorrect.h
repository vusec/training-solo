 ...
 * an incorrect prediction on the iBTB
 *
 * For reference, single run hit results:
 * | CPU                    |   SHORT            NEAR             FAR
 * |------------------------|--------------------------------------------------|
 * | Intel Core i7-10700K   |      0 (  0.0%)   1600 ( 16.0%)   8400 ( 84.0%)
 * | Intel Core i7-11700    |      0 (  0.0%)   1623 ( 16.2%)   8369 ( 83.7%)
 * | Intel Core i9-14900K P |      0 (  0.0%)    357 (  3.6%)   9643 ( 96.4%)
 * | Intel Core 7 258V P    |      0 (  0.0%)    403 (  4.0%)   9579 ( 95.8%)
 */
...
void do_train_function(config * cfg) {
    ...
    // Insert incorrect iBTB Entry (iBTB=incorrect)
    execute_path(cfg, &cfg->histories[P_VICTIM], &cfg->paths[P_TRAINING], &cfg->target1_mock);
    execute_path(cfg, &cfg->histories[P_VICTIM], &cfg->paths[P_TRAINING], &cfg->target1_mock);

    // we need to evict the pc entry, to make sure correct BTB entry is
    // inserted later
    evict_btb_entry_caller1(cfg);
    evict_btb_entry_caller1(cfg);
