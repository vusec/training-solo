...
void print_mitigation_info(int cpu) {
    ...
    printf("%22s: %2d | %22s  %2s\n",
        "IBRS/IBPB Support",  get_bit((uint32_t) rdmsr(MSR_IA32_SPEC_CTRL, cpu), BIT_RRSBA_DIS_S),
        "", "");

    printf("%22s: %2d | %22s: %2d\n",
        "eIBRS Support", get_bit((uint32_t) rdmsr(MSR_IA32_ARCH_CAPABILITIES, cpu), BIT_IBRS_ALL),
        "eIBRS Enabled", get_bit((uint32_t) rdmsr(MSR_IA32_SPEC_CTRL, cpu), BIT_IBRS_MSR));

    printf("%22s: %2d | %22s  %2s\n",
        "RSBA", get_bit((uint32_t) rdmsr(MSR_IA32_ARCH_CAPABILITIES, cpu), BIT_RSBA),
        "", "");

    printf("%22s: %2d | %22s  %2s\n",
        "RRSBA", get_bit((uint32_t) rdmsr(MSR_IA32_ARCH_CAPABILITIES, cpu), BIT_RRSBA),
        "", "");
    ...
    printf("%22s: %2d | %22s: %2d\n",
        ...
        "IBT Kernel-mode", ibt_support ? get_bit((uint32_t) rdmsr(MSR_IA32_S_CET, cpu), CET_ENDBR_EN) : 0);

    printf("%22s  %2s | %22s: %2d\n",
        "", "",
        "IBT User-mode", ibt_support ? get_bit((uint32_t) rdmsr(MSR_IA32_U_CET, cpu), CET_ENDBR_EN) : 0);
    ...
    printf("%22s: %2d | %22s: %2d\n",
        ...
        "RRSBA_DIS_S", get_bit((uint32_t) rdmsr(MSR_IA32_SPEC_CTRL, cpu), BIT_RRSBA_DIS_S));

    printf("%22s  %2s | %22s: %2d\n",
        "", "",
        "RRSBA_DIS_U", get_bit((uint32_t) rdmsr(MSR_IA32_SPEC_CTRL, cpu), BIT_RRSBA_DIS_U));

    printf("%22s: %2d | %22s: %2d\n",
        "DDP_CTRL", get_bit(edx, BIT_DDP_CTRL),
        "DDPD_U", get_bit((uint32_t) rdmsr(MSR_IA32_SPEC_CTRL, cpu), BIT_DDPD_U));

    printf("%22s: %2d | %22s: %2d\n",
        "BHI_CTRL", get_bit(edx, BIT_BHI_CTRL),
        "BHI_DIS_S", get_bit((uint32_t) rdmsr(MSR_IA32_SPEC_CTRL, cpu), BIT_BHI_DIS_S));

    printf("%22s: %2d | %22s  %2s\n",
        "BHI_NO", get_bit((uint32_t) rdmsr(MSR_IA32_ARCH_CAPABILITIES, cpu), BIT_BHI_NO),
        "", "");
